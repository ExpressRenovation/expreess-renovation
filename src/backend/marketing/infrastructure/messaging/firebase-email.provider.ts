...
export class FirebaseEmailProvider implements MessagingService {
    ...
    async sendEmail(leadId: string, templateId: string, variables: Record<string, string>): Promise<void> {
        ...
        const textContent = variables.textBody || `Mensaje automático (plantilla: ${templateId})`;
        ...
        await db.collection('mail').add({
            to: email,
            message: {
                subject: 'Siguientes pasos con Express Renovation Mallorca',
                text: textContent,
                html: `<div style="font-family: 'Helvetica Neue', Arial, sans-serif; color: #1a1a1a; max-width: 600px; line-height: 1.6;">
                         ${htmlContent}
                         <br>
                         <hr style="border: none; border-top: 1px solid #eaeaea; margin: 20px 0;">
                         <p style="color: #666; font-size: 12px;"><small><strong>Express Renovation Mallorca</strong> - Este es un aviso automático.</small></p>
                         ${trackingPixel}
                       </div>`
            },
