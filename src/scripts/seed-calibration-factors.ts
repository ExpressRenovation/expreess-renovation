 ...
 *   - chapter DEMOLICIONES 1.42 (small-surface demolition evidence), source "seed"
 *   - guard { min_samples: 8, clamp_min: 0.8, clamp_max: 2.6, outlier_ratio_cap: 3.0 }
 *
 * Uso:
 *   npx tsx src/scripts/seed-calibration-factors.ts
 *
 * ⚠️ El write es incondicional al proyecto que resuelvan las credenciales.
 * CONFIRMA que el entorno apunta a `express-renovation` (FIREBASE_PROJECT_ID /
 * GCLOUD_PROJECT), NO a un proyecto de dev (p.ej. local-digital-eye), antes de
 * correrlo. Alternativa fiable: sembrar con proyecto explícito vía script Python.
 *
