        ...
        try {
            ...
            if (isTraceMode) {
                ...
                if (result.success) {
                    ...
                    // --- Background RLHF Telemetry para el Admin (No bloqueante) ---
                    import('@/actions/budget/save-admin-correction-trace.action').then(m => {
                        m.saveAdminCorrectionTraceAction(
                            budget.id,
                            budget, // Baseline original desde Props
                            finalJson,
                            msSinceLoad
                        ).catch(console.error);
                    });
    ...
    const handlePdfDownloaded = async () => {
        if (!isAdmin && budget.id) { // In demo mode, budget.id is actually the traceId
            try {
                ...
                // This used to push to `/es/wizard/success`, a route that never
                // existed — the page lived under /dashboard/wizard/success and
                // has since been removed (it carried another product's copy).
                // Nothing to redirect to after a download, so we stay put.
            } catch (error) {
