...
const BooleanItem = ({ label, value, detail }: { label: string, value: boolean, detail?: string }) => (
    <div className="flex justify-between py-1.5 border-b border-slate-50 last:border-0">
        ...
        <div className="flex items-center gap-2">
            {detail && value && <span className="text-slate-700 font-mono text-xs">{detail}</span>}
            {value ? (
                ...
                <span className="text-slate-300 font-medium text-xs">No</span>
            )}
        </div>
    </div>
);
    ...
    return (
        <Card className="border-0 shadow-none bg-transparent">
            <CardHeader className="px-0 pt-0">
                <CardTitle className="text-lg text-slate-800">Datos Completos de la Solicitud</CardTitle>
            </CardHeader>
            <CardContent className="px-0 space-y-8">

                {/* Contact Info Card */}
                <div className="bg-white p-5 rounded-xl border shadow-sm">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 border-b pb-2">Cliente y Proyecto</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                        <div className="space-y-1">
                            <div className="font-semibold text-base text-slate-900">{data.name}</div>
                            <div className="text-slate-500">{data.email}</div>
                            <div className="text-slate-500">{data.phone}</div>
                        </div>
                        <div className="text-right space-y-1">
                            <div className="font-medium text-slate-900">{data.address}</div>
                            ...
                            <div className="font-mono text-slate-600 bg-slate-100 px-2 py-0.5 rounded inline-block mt-1">
                                {data.totalAreaM2} m²
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Distribución</h4>
                        ...
                        {/* Demoliciones */}
                        <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Demoliciones Previas</h4>
                            <div className="space-y-1 text-sm">
                        ...
                        <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Albañilería y Techos</h4>
                            <div className="space-y-1 text-sm">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Carpintería */}
                        <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Carpintería</h4>
                            <div className="space-y-1 text-sm">
                        ...
                        {/* Instalaciones */}
                        <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Instalaciones</h4>
                            <div className="space-y-1 text-sm">
                        ...
                        {/* Acabados */}
                        <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Acabados</h4>
                            <div className="space-y-1 text-sm">
                ...
                {data.kitchen?.renovate && (
                    <div className="bg-white p-5 rounded-xl border shadow-sm space-y-3 border-l-4 border-l-amber-400">
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Reforma de Cocina</h4>
                            <span className="text-xs font-bold bg-amber-100 text-amber-700 px-2 py-0.5 rounded uppercase">{data.kitchen.quality || 'Estándar'}</span>
                        </div>
                ...
                {data.files && data.files.length > 0 && (
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 text-sm">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Archivos Adjuntos</span>
                        <div className="flex gap-2">
                            {data.files.map((file, i) => (
                                <a key={i} href={file} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline text-xs truncate max-w-[200px] block">
                                    Archivo {i + 1}
                                </a>
                            ))}
                        </div>
